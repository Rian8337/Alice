...
module.exports.run = (oldMessage, newMessage) => {
    if (oldMessage.author.bot || oldMessage.channel instanceof Discord.DMChannel) {
		return;
	}
	if (oldMessage.content == newMessage.content) {
		return;
	}
	const logchannel = oldMessage.guild.channels.cache.find((c) => c.name === config.log_channel);
	if (!logchannel) {
		return;
	}
