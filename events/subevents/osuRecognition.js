module.exports.run = (client, message, current_map) => {
    ...
    for (let i = 0; i < msgArray.length; i++) {
        if (!msgArray[i].startsWith("https://osu.ppy.sh/") && !msgArray[i].startsWith("https://bloodcat.com/osu")) continue;
        ...
        else if (msgArray[i].indexOf("/beatmapsets/") !== -1 || msgArray[i].indexOf("/s/") !== -1 || msgArray[i].indexOf("/m/") !== -1) client.utils.get("autocalc").run(client, message, msgArray.slice(i), current_map, true)
    }
};
