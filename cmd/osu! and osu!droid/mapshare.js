...
function editList(res, page, color, footer, index) {
    ...
    binddb.findOne(query, async (err, userres) => {
        ...
        switch (args[0]) {
            case "list": {
                ...
                if (!["accepted", "denied", "pending", "posted"].includes(status)) {
                    return message.channel.send("❎ **| Hey, that's an invalid submission status! Accepted statuses are `accepted`, `denied`, `pending`, and `posted`.**")
                }
                mapdb.find({status: status}).toArray((err, map_entries) => {
                    ...
                    if (map_entries.length === 0) {
                        return message.channel.send(`❎ **| I'm sorry, there is no submission with ${status} status now!**`);
                    }
                    if (!map_entries[(page - 1)*10]) {
                        return message.channel.send(`❎ **| Hey, we don't have that much submission for ${status} status!**`);
                    }
                    const color = message.member.roles.color?.hexColor || "#000000";
                    const footer = config.avatar_list;
                    const index = Math.floor(Math.random() * footer.length);
                    let embed = editList(map_entries, page, color, footer, index);
                    message.channel.send({embed: embed}).then(msg => {
                        ...
                        backward.on('collect', () => {
                            ...
                            embed = editList(map_entries, page, color, footer, index);
                        ...
                        forward.on('collect', () => {
                            ...
                            msg.reactions.cache.forEach((reaction) => reaction.users.remove(message.author.id).catch(console.error));
                            embed = editList(res, page, color, footer, index);
                            msg.edit({embed: embed}).catch(console.error);
                        });
            ...
            case "post": {
                if (!message.isOwner) {
                    ...
                    if (!isMapShareChannel) {
                        return message.channel.send("❎ **| I'm sorry, this command is not available in this channel.**");
                    }
                    if (!message.member.roles.cache.has('715219617303232542')) {
                ...
                mapdb.findOne(query, async (err, res) => {
                    ...
                    if (!res) {
                        return message.channel.send("❎ **| I'm sorry, there is no submission with that beatmap!**");
                    }
                    ...
                    if (submitter) {
                        embed.setAuthor(`Submission by ${submitter.user.username}`, submitter.user.avatarURL({dynamic: true}));
                    }
            ...
            case "deny": {
                if (!message.isOwner) {
                    ...
                    if (!message.member.roles.cache.has('715219617303232542')) {
                        return message.channel.send("❎ **| I'm sorry, you don't have permission to use this command.**");
                    }
                }
                let beatmap_id = args[1];
                if (typeof beatmap_id === 'string') {
                ...
                mapdb.findOne(query, (err, res) => {
                    ...
                    if (res.status !== "pending") {
                        return message.channel.send("❎ **| I'm sorry, the submission with that beatmap has been processed before!**");
                    }
                    updateVal = {
                        $set: {
                            status: "denied"
            ...
            case "view": {
                ...
                mapdb.findOne(query, async (err, res) => {
                    ...
                    const mapinfo = await osudroid.MapInfo.getInformation({beatmapID: beatmap_id});
                    if (mapinfo.error) {
                        return message.channel.send("❎ **| I'm sorry, I couldn't fetch beatmap info from osu! API! Perhaps it is down?**");
                    }
                    if (!mapinfo.title) {
                        mapdb.deleteOne(query, err => {
                            if (err) {
                    ...
                    if (submitter) {
                        embed.setAuthor(`Submission by ${submitter.user.tag}`, submitter.user.avatarURL({dynamic: true}));
                    }
            ...
            case "ban": {
                ...
                if (!message.isOwner && isEligible(message.member) !== 0) {
                    return message.channel.send("❎ **| I'm sorry, you don't have the permission to use this command.**");
                }
                if (!isMapShareChannel) {
                    return message.channel.send("❎ **| I'm sorry, this command is not available in this channel.**");
                ...
                if (!user) {
                    return message.channel.send("❎ **| Hey, please enter a valid user to ban from map sharing!**");
                }
                ...
                pointdb.findOne(query, (err, pres) => {
                    ...
                    if (pres) {
                        if (pres.isBannedFromMapShare) {
                            return message.channel.send("❎ **| I'm sorry, the user is already banned from map sharing!**");
                        }
            ...
            case "unban": {
                ...
                pointdb.findOne(query, (err, pres) => {
                    ...
                    if (pres) {
                        if (!pres.isBannedFromMapShare) {
                            return message.channel.send("❎ **| I'm sorry, the user is not banned from map sharing!**");
                        }
                        ...
                        pointdb.updateOne(query, updateVal, err => {
                            ...
                            message.channel.send("✅ **| Successfully unbanned user from map sharing.**");
                        });
                    } else {
                        insertVal = {
                            username: userres.username,
            ...
            default: {
                if (!(message.channel instanceof Discord.DMChannel)) {
                    return message.channel.send("❎ **| I'm sorry, this part of the command is only allowed in DMs.**");
                }
                if (!message.isOwner) {
                    const inter = client.guilds.cache.get("316545691545501706");
                    const member = inter.member(message.author);
                    if (!member) {
                        return message.channel.send("❎ **| I'm sorry, you are not a member of osu!droid International server!**");
                    }
                    if (member.roles.cache.has('715219617303232542')) {
                        return message.channel.send("❎ **| I'm sorry, you cannot submit a beatmap recommendation as a Map-share Manager!**");
                    }
                }
                pointdb.findOne(query, async (err, pres) => {
                    ...
                    if (pres) {
                        if (pres.isBannedFromMapShare) {
                            return message.channel.send("❎ **| I'm sorry, you were banned from sharing maps due to abuse of map sharing system!**");
                        }
                        if (pres.hasSubmittedMapShare) {
                            return message.channel.send("❎ **| I'm sorry, you have submitted a map for this day! Please submit in the next day!**");
                        }
                    }
                    ...
                    if (mapinfo.objects < 50) {
                        return message.channel.send("❎ **| I'm sorry, it seems like the beatmap has less than 50 objects!**");
                    }
                    if (mapinfo.circles + mapinfo.sliders === 0 || mapinfo.total_length < 30) {
                        return message.channel.send("❎ **| Hey, please refrain from posting troll submissions!**");
                    }

                    if (mapinfo.approved === osudroid.rankedStatus.WIP || mapinfo.approved === osudroid.rankedStatus.QUALIFIED) {
                        return message.channel.send("❎ **| I'm sorry, you cannot submit a WIP (Work In Progress) and qualified beatmap!**");
                    }

                    if (mapinfo.favorites > 250) {
                        return message.channel.send("❎ **| I'm sorry, that beatmap has over 250 favorites!**");
                    }
                    if (mapinfo.plays > 300000) {
                        return message.channel.send("❎ **| I'm sorry, that beatmap has over 300000 plays!**");
                    }
                    ...
                    if (mapinfo.approved !== osudroid.rankedStatus.RANKED) {
                        if (date.getTime() - mapinfo.submitDate.getTime() < 86400 * 1000 * 7) {
                            return message.channel.send("❎ **| I'm sorry, that beatmap was submitted less than a week ago!**");
                        }
                        if (date.getTime() - mapinfo.lastUpdate.getTime() < 86400 * 1000 * 3) {
                            return message.channel.send("❎ **| I'm sorry, that beatmap was updated less than 3 days ago!**");
                        }
                    }

                    if (mapinfo.diff_total < 3) {
                        return message.channel.send("❎ **| I'm sorry, you can only submit beatmaps that are 3\* or higher!**");
                    }

                    const cmd_length = message.content.split(" ").slice(0, 2).join(" ").length + 1;
                    let summary = message.content.substring(cmd_length);
                    if (!summary) {
                        return message.channel.send("❎ **| Hey, please enter a summary of the beatmap you are picking!**");
                    }
                    const words = summary.split(" ").length;
                    if (words < 50) {
                        return message.channel.send(`❎ **| I'm sorry, your summary is too short (${words}/50 words)!**`);
                    }
                    if (words> 120) {
                        return message.channel.send(`❎ **| I'm sorry, your summary exceeded 120 words (${words}/120)!**`);
                    }
                    if (summary.length > 900) {
                        return message.channel.send("❎ **| I'm sorry, your summary must be at least 100 characters and at most 900 characters!**");
                    }
                    ...
                    mapdb.findOne({beatmap_id: mapinfo.beatmapID}, async (err, mres) => {
                        ...
                        if (mres) {
                            return message.channel.send("❎ **| I'm sorry, someone has submitted the beatmap before!**");
                        }

                        const mapList = await retrieveBeatmapList(mapinfo.beatmapsetID);
                        if (!mapList) {
                            return message.channel.send("❎ **| I'm sorry, I couldn't fetch beatmap set information! Perhaps osu! API is down?**");
                        }
                        ...
                        mapdb.find(map_query).toArray((err, map_list) => {
                            ...
                            if (map_list.length > 0) {
                                for (const map of map_list) {
                                    ...
                                    if (mapinfo.objects === object_count) {
                                        return message.channel.send("❎ **| Hey, please refrain from submitting duplicated map submissions!**");
                                    }
                                }
                            }
