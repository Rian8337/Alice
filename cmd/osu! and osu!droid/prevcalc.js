...
module.exports.run = async (client, message, args, maindb, alicedb, current_map) => {
	...
	const embed = new Discord.MessageEmbed()
		...
		.addField(`**Droid pp (Experimental)**: __${(ppline / 20).toFixed(2)} pp__${isEstimatedValue && acc !== 100 ? " (estimated)" : ""} - ${starsline} stars`, `**PC pp**: ${(pcppline / 20).toFixed(2)} pp${isEstimatedValue && acc !== 100 ? " (estimated)" : ""} - ${pcstarsline} stars`);
