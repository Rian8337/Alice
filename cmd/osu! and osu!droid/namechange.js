...
module.exports.run = (client, message, args, maindb, alicedb) => {
    ...
    switch (args[0]) {
        case 'accept': {
            if (!message.isOwner) {
                return message.channel.send("❎ **| I'm sorry, you don't have the permission to do this.**");
            }
            ...
            namedb.findOne(query, async (err, res) => {
                ...
                if (!res || res.isProcessed) {
                    return message.channel.send("❎ **| I'm sorry, this user does not have an active name change request!**");
                }
        ...
        case 'deny': {
            ...
            namedb.findOne(query, async (err, res) => {
                ...
                const user = await guild.members.fetch(res.discordid).catch();
                let cooldown = res.cooldown;
                let old_name = res.current_username;
                let new_name = res.new_username;
                ...
                namedb.updateOne(query, updateVal, err => {
                    ...
                    if (!user) {
                        return message.channel.send(`✅ **| Successfully denied request with reason \`${reason}\`, however this user is not in the server!**`);
                    }
        ...
        case 'list': {
            namedb.find({isProcessed: false}).sort({cooldown: 1}).toArray((err, res) => {
                ...
                if (res.length === 0) {
                    return message.channel.send("❎ **| I'm sorry, there is no active name change request now!**");
                }
                
                let page = 1;
                embed = processEmbed(res, page, footer, index);
                message.channel.send({embed: embed}).then(msg => {
                    if (Math.ceil(res.length / 10) === page) {
                        return;
                    }
                    msg.react("⏮️").then(() => {
                        msg.react("⬅️").then(() => {
        ...
        default: {
            ...
            if (args.length > 2) {
                return message.channel.send("❎ **| Hey, spaces in nicknames are not allowed!**");
            }

            const email = args[0];
            if (!email) {
                return message.channel.send("❎ **| Hey, please enter your email address!**");
            }

            const new_name = args[1];
            if (!new_name) {
                return message.channel.send("❎ **| Hey, please enter the desired new nickname that you want to use!**");
            }
            if (new_name.length < 2 || new_name.length > 20) {
                return message.channel.send("❎ **| I'm sorry, a username must be at least 2 characters and doesn't exceed 20 characters!**");
            }
            if (hasUnicode(new_name) || !(/^[a-zA-Z0-9_]+$/.test(new_name))) {
                return message.channel.send("❎ **| I'm sorry, usernames can only contain letters, numbers, and underscores!**");
            }
            if (new_name.includes('<:')) {
                return message.channel.send("❎ **| I'm sorry, a username cannot contain emojis!**");
            }

            if (message.attachments.size === 0) {
                return message.channel.send("❎ **| Hey, please attach a screenshot of your osu!droid main menu with your account logged in along with the command!**");
            }
            if (message.attachments.size > 1) {
                return message.channel.send("❎ **| Hey, please attach only one screenshot of your osu!droid main menu with your account logged in!**");
            }
            ...
            binddb.findOne(query, (err, res) => {
                ...
                namedb.findOne(query, async (err, nameres) => {
                    ...
                    let cooldown = 0;
                    if (nameres) {
                        if (!nameres.isProcessed) {
                            return message.channel.send("❎ **| Hey, you currently have an active request! Please wait for that one to get reviewed before submitting another one!**");
                        }
                        cooldown = nameres.cooldown;
                        if (curtime < cooldown) {
                            return message.channel.send(`❎ **| I'm sorry, you're still in cooldown! You will be able to send a name change request in \`${new Date(cooldown * 1000).toUTCString()}\`.**`);
                        }
                    }
                    ...
                    if (email !== player.email) {
                        return message.channel.send("❎ **| I'm sorry, the email you have provided is not the same as the email registered to your binded osu!droid account!**");
                    }
                    if (username !== player.username) {
                        return message.channel.send("❎ **| I'm sorry, your username is not the same as the one stored in bot database! If you've requested a name change before, please rebind your account using `a!userbind <uid>` and then submit a request again!**");
                    }

                    const new_player = await osudroid.Player.getInformation({username: new_name});
                    if (new_player.error) {
                        return message.channel.send("❎ **| I'm sorry, I couldn't check for nickname availability! Perhaps osu!droid server is down?**");
                    }
