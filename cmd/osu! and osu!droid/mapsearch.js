...
module.exports.config = {
    name: "mapsearch",
    description: "Searches for beatmaps with specified query.\n\n**Important note: In artist, title, creator, difficulty name, or source parameter, replace spaces with `%20` and separate multiple genres, languages, and ranking status with commas**.",
    ...
    detail: "`artist`: Artist to search [String]\n`creator`: Creator (mapper) to search [String]\n`diff name`: Difficulty name to search [String]\n`genre`: Genre to search. Accepted genres are \`Anime\`, \`VideoGame\`, \`Novelty\`, \`Electronic\`, \`Pop\`, \`Rock\`, \`HipHop\`, \`Other\`, and \`Any\` (case insensitive) [String]\n`language`: Language to search. Accepted languages are \`JP\` (Japanese), \`Ins\` (Instrumental), \`ENG\` (English), \`KR\` (Korean), \`CN\` (Chinese), \`DE\` (German), \`SP\` (Spanish), \`IT\` (Italian), \`FR\` (French), \`Other\`, and \`Any\` (case insensitive) [String]\n`max/min ar`: Maximum/minimum AR to search [Float]\n`max/min bpm`: Maximum/minimum BPM to search [Float]\n`max/min cs`: Maximum/minimum CS to search [Float]\n`max/min fav`: Maximum/minimum favorite count to search [Integer]\n`max/min hp`: Maximum/minimum HP to search [Float]\n`max/min length`: Maximum/minimum map length to search in seconds [Integer]\n`max/min pc`: Maximum/minimum play count to search [Integer]\n`max/min sr`: Maximum/minimum star rating to search [Float]\n`end/start date`: Maximum/minimum date to search in `YYYY-MM-DD` format [String]\n`status`: Ranking status to search. Accepted ranking statuses are \`R\` (Ranked), \`Q\` (Qualified), \`L\` (Loved), and \`U\` (Unranked) [String]",
    permission: "None"
};