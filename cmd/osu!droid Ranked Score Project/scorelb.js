...
function editscore(res, page) {
    ...
    for (let i = 20 * (page - 1); i < 20 + 20 * (page - 1); i++) {
        if (res[i]) {
            output += spaceFill((i+1).toString(),4) + ' | ' + spaceFill(res[i].username, 17) + ' | ' + spaceFill(res[i].uid.toString(), 7) + ' | ' + spaceFill(res[i].playc.toString(), 6) + ' | ' + parseInt(res[i].score).toLocaleString() + ' (' + Math.floor(res[i].level).toString() + ')\n';
        } else {
