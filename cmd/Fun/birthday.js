...
module.exports.run = (client, message, args, maindb, alicedb) => {
    ...
    if (isNaN(month) || month < 0 || month > 11) {
        return message.channel.send("❎ **| Hey, that's an invalid month!**");
    }

    let date = parseInt(args[1]);
    if (isNaN(date) || date < 1 || date > 31) {
        return message.channel.send("❎ **| Hey, that's an invalid date!**");
    }
...
module.exports.config = {
    name: "birthday",
    description: "Sets your birthday date. Get 1000 Alice coins and a happy birthday role in the international server on your birthday! Once set, a birthday date cannot be changed.\n\nIf someone's birthday is February 29, it will be celebrated in March 1 in nonleap years.",
