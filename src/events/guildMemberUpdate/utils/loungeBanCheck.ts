...
export const run: EventUtil["run"] = async (_, member: GuildMember) => {
    ...
    const role: Role | undefined = member.guild.roles.cache.find(r => r.name === "Lounge Pass");
    ...
    member.roles.remove(role, `Locked from lounge channel for \`${lockInfo.reason ?? "not specified"}\``);

    const embed: MessageEmbed = EmbedCreator.createNormalEmbed(
        { color: "#b58d3c", timestamp: true }
    );

    embed.setDescription(
        `${member} is locked from lounge channel!\n` +
        `Reason: ${lockInfo.reason ?? "Not specified"}.\n\n` +
        `This lock will ${!Number.isFinite(lockInfo.expiration) ? "not expire" : `expire at ${new Date(lockInfo.expiration).toUTCString()}`}.`
    );

    (<TextChannel> member.guild.channels.resolve("783506454966566912")).send({ embeds: [embed] });
};

export const config: EventUtil["config"] = {
    description: "Responsible for checking if a user is locked from lounge channel upon giving lounge pass role.",
