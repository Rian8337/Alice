...
export const run: Event["run"] = async (client, threads: Collection<Snowflake, ThreadChannel>) => {
    EventHelper.runUtilities(client, __dirname, threads.first()?.guild, threads.first()?.parent, threads)
