...
export class UserBindCollectionManager extends DatabaseCollectionManager<DatabaseUserBind, UserBind> {
     ...
     * @returns The players.
     */
    async getDPPUnscannedPlayers(amount: number): Promise<DiscordCollection<Snowflake, UserBind>> {
     ...
     * @returns The players.
     */
    async getRecalcUnscannedPlayers(amount: number): Promise<DiscordCollection<Snowflake, UserBind>> {
        const userBind: DatabaseUserBind[] = await this.collection.find(
