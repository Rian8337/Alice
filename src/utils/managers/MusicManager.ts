...
export abstract class MusicManager extends Manager {
     ...
     */
    static async enqueue(channel: VoiceChannel | StageChannel, executionChannel: TextChannel | ThreadChannel, queue: MusicQueue, index?: number): Promise<OperationResult> {
        let musicInformation: MusicInfo | undefined = this.musicInformations.get(channel.guildId);
        ...
        try {
            await entersState(musicInformation.connection, VoiceConnectionStatus.Ready, 2e4);
        } catch (error) {
            this.client.emit("error", <Error> error);

            return this.createOperationResult(false, "failed to join voice channel within 20 seconds");
        }
    ...
    static pause(channel: VoiceChannel | StageChannel): OperationResult {
        ...
        if (musicInformation.player.state.status !== AudioPlayerStatus.Playing) {
     ...
     * @returns An object containing information about the operation.
     */
    static async skip(channel: VoiceChannel | StageChannel): Promise<OperationResult> {
        const musicInformation: MusicInfo | undefined = this.musicInformations.get(channel.guildId);
