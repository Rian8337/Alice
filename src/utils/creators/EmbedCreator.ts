...
export abstract class EmbedCreator {
    ...
    static createNormalEmbed(
        ...
        if (embedOptions.author) {
            embed.setAuthor(
                embedOptions.author.tag,
                <string>embedOptions.author.avatarURL({ dynamic: true })
            );
        }
        ...
        return {
            embeds: [
                embed
                    .setAuthor(
                        "Beatmap Information",
                        `attachment://osu-${beatmapInfo.totalDifficulty.toFixed(
                            2
                        )}.png`
                    )
                    .setThumbnail(
    ...
    static async createCalculationEmbed(
        ...
        if (
            ...
            embed
                ...
                .addField(
                    `**Star Rating**`,
                    `${Symbols.star.repeat(
                        Math.min(10, Math.floor(calculationResult.droid.total))
        ...
        if (
            !Precision.almostEqualsNumber(
                calculationResult.map.totalDifficulty,
                newRating.total
            )
        ) {
            ...
            embed.setAuthor(
                "Beatmap Information",
                `attachment://osu-${newRating.total.toFixed(2)}.png`
            );
        }
    ...
    static async createRecentPlayEmbed(
        ...
        if (score.accuracy.nmiss > 0 || score.combo < calcResult.map.maxCombo) {
            ...
            const fcCalcResult: PerformanceCalculationResult =
                (await BeatmapDifficultyHelper.calculateBeatmapPerformance(
                    new StarRatingCalculationResult(
                        calcResult.map,
                        calcResult.droid.stars,
                        calcResult.osu.stars
                    ),
                    calcParams
                ))!;

            beatmapInformation += `(${fcCalcResult.droid.total.toFixed(
                2
            )}DPP, ${fcCalcResult.osu.total.toFixed(2)}PP for ${(
    ...
    static async createMapShareEmbed(
        ...
        embed
            ...
            .setAuthor(
                `Submission by ${submission.submitter}`,
                `attachment://osu-${calcResult.osu.total.toFixed(2)}.png`
            )
            .addField(
                "**Star Rating**",
