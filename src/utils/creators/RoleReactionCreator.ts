...
import { Guild, GuildMember, Message, ReactionCollector, Role, Snowflake } from "discord.js";
...
export abstract class RoleReactionCreator extends Manager {
     ...
     */
    static async createReactionBasedRoleAssignment(guild: Guild, message: Message, role: Role, emoji: Symbols | Snowflake): Promise<ReactionCollector> {
        await message.react(emoji);

        const collector: ReactionCollector = message.createReactionCollector({
            filter: (reaction, user) => (reaction.emoji.name === emoji || reaction.emoji.id === emoji) && user.id !== this.client.user?.id
        });
        ...
        collector.on("collect", async (_, user) => {
            ...
            message.reactions.cache.forEach(reaction => reaction.users.remove(user.id));
        });
