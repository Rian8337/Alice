...
export const run: Command["run"] = async (_, interaction) => {
    ...
    if (!stats) {
        return interaction.editReply({
            content: MessageCreator.createReject(emojistatisticsStrings.serverHasNoData)
        });
    }
    ...
    for await (const emoji of stats.emojiStats.values()) {
        ...
        const months: number = Math.max(1, (currentDate.getUTCFullYear() - dateCreation.getUTCFullYear()) * 12 + currentDate.getUTCMonth() - dateCreation.getUTCMonth());
    ...
    if (validEmojis.length === 0) {
        return interaction.editReply({
            content: MessageCreator.createReject(emojistatisticsStrings.noValidEmojis)
        });
    }

    const sortOption: string = interaction.options.getString("sortoption") ?? "overall";
    ...
    embed.setAuthor(`Emoji Statistics for ${interaction.guild!.name}`, interaction.guild!.iconURL({ dynamic: true })!)
        .setDescription(`**Sort Mode**: ${sortOption === "overall" ? "Overall" : "Average per month"} usage`);
    ...
    const onPageChange: OnButtonPageChange = async (_, page) => {
        for (let i = 5 * (page - 1); i < Math.min(validEmojis.length, 5 + 5 * (page - 1)); ++i) {
            embed.addField(
                ...
                `**Date Creation**: ${validEmojis[i].emoji.createdAt.toUTCString()}\n` +
                `**Overall Usage**: ${validEmojis[i].count.toLocaleString()}\n` +
                `**Average Per Month Usage**: ${validEmojis[i].averagePerMonth.toLocaleString()}`
            );
        }
    };
