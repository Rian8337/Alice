...
export const run: Subcommand["run"] = async (client, interaction) => {
    const pick: string = interaction.options.getString("pick", true).toUpperCase();

    const match: TournamentMatch | null = await DatabaseManager.elainaDb.collections.tournamentMatch.getByChannel(interaction.channelId);
    ...
    const timeLimit: number = Math.ceil(parseInt(<string> map[1]) / (pick.includes("DT") ? 1.5 : 1));

    const embed: MessageEmbed = EmbedCreator.createNormalEmbed(
        { timestamp: true, color: (<GuildMember> interaction.member).displayColor }
    );
    ...
    setTimeout(() => {
        ...
        interaction.channel!.send({
            content: MessageCreator.createAccept(matchStrings.roundCountdownFinished)
        });
    }, timeLimit * 1000);
};
