...
export const run: Subcommand["run"] = async (_, interaction) => {
    ...
    const score: Score = await Score.getFromHash({ uid: bindInfo.uid, hash: beatmapInfo.hash });

    if (!score.title) {
        return interaction.editReply({
            content: MessageCreator.createReject(submitStrings.noScoreSubmitted)
        });
    }
    ...
    switch (submissionValidity) {
        ...
        case DPPSubmissionValidity.BEATMAP_NOT_WHITELISTED:
            return interaction.editReply({
                content: MessageCreator.createReject(submitStrings.beatmapNotWhitelisted)
            });
        case DPPSubmissionValidity.SCORE_USES_FORCE_AR:
            return interaction.editReply({
                content: MessageCreator.createReject(submitStrings.scoreUsesForceAR)
            });
        case DPPSubmissionValidity.SCORE_USES_CUSTOM_SPEED:
            return interaction.editReply({
                content: MessageCreator.createReject(submitStrings.scoreUsesCustomSpeedMultiplier)
            });
    }
    ...
    if (RankedScoreHelper.isBeatmapEligible(beatmapInfo.approved)) {
        ...
        if (rankedScoreInfo) {
            ...
            const doc: DatabaseRankedScore = rankedScoreDbManager.defaultDocument;
