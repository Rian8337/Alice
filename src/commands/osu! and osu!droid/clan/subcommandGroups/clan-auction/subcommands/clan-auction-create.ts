...
export const run: Subcommand["run"] = async (client, interaction) => {
    ...
    const powerup: PowerupType = <PowerupType> interaction.options.getString("powerup", true);

    const amount: number = interaction.options.getInteger("amount", true);

    const minBidAmount: number = interaction.options.getInteger("minimumbidamount", true);

    const duration: number = CommandHelper.convertStringTimeFormat(interaction.options.getString("duration", true));

    if (name.length > 20) {
        return interaction.editReply({
            content: MessageCreator.createReject(clanStrings.clanAuctionNameIsTooLong)
        });
    }
    ...
    if (!NumberHelper.isNumberInRange(duration, 60, 86400, true)) {
        return interaction.editReply({
            content: MessageCreator.createReject(clanStrings.invalidClanAuctionDuration)
        });
    }
    ...
    const auctionCheck: ClanAuction | null = await DatabaseManager.aliceDb.collections.clanAuction.getFromName(name);
    ...
    const confirmation: boolean = await MessageButtonCreator.createConfirmation(
        interaction,
        {
            content: MessageCreator.createWarn(clanStrings.clanAuctionCreateConfirmation)
        },
        [interaction.user.id],
        20
    );
    ...
    if (!result.success) {
        return interaction.editReply({
            content: MessageCreator.createReject(
                clanStrings.clanAuctionCreateFailed, result.reason!
            )
        });
    }
    ...
    const embed: MessageEmbed = EmbedCreator.createClanAuctionEmbed(newAuction, coinEmoji);

    embed.spliceFields(embed.fields.length - 1, 1);

    await notificationChannel.send({
        content: MessageCreator.createWarn("An auction has started with the following details:"),
        embeds: [ embed ]
    });
