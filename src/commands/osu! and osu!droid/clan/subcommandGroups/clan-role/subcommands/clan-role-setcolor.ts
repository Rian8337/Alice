...
export const run: Subcommand["run"] = async (_, interaction) => {
    ...
    if (interaction.options.getString("color") && !StringHelper.isValidHexCode(color)) {
    ...
    if (!clan.roleColorUnlocked) {
        return interaction.editReply({
            content: MessageCreator.createReject(clanStrings.roleColorIsNotUnlocked)
        });
    }
    ...
    interaction.editReply({
        content: MessageCreator.createAccept(clanStrings.changeRoleColorSuccessful)
    });
};
