...
export const run: Subcommand["run"] = async (_, interaction) => {
    ...
    const upkeep: number = clan.individualUpkeepBaseValue + clan.calculateUpkeep(interaction.user);
    const totalUpkeep: number = clan.calculateOverallUpkeep();

    interaction.editReply({
        content: MessageCreator.createAccept(
            clanStrings.clanUpkeepInformation,
            clan.upkeepBaseValue % clan.member_list.size ? `somewhere between ${upkeep.toLocaleString()}-${(upkeep + 1).toLocaleString()}` : upkeep.toLocaleString(),
