    ...
    if (!bindInfo) {
        return interaction.editReply({
            content: MessageCreator.createReject(
                interaction.options.getUser("user") ? Constants.userNotBindedReject : Constants.selfNotBindedReject
            )
        });
    }
    ...
    switch (type) {
        case "battle":
            if (!bindInfo.clan) {
                return interaction.editReply({
                    content: MessageCreator.createReject(
                        interaction.options.getUser("user") ? clanStrings.userIsNotInClan : clanStrings.selfIsNotInClan
                    )
                });
            }

            const clan: Clan = (await DatabaseManager.elainaDb.collections.clan.getFromUser(user))!;

            const member: ClanMember = clan.member_list.get(user.id)!;

            const battleCooldownDifference: number = DateTimeFormatHelper.getTimeDifference(member.battle_cooldown);
            ...
            if (battleCooldownDifference > 0) {
                interaction.editReply({
                    content: MessageCreator.createAccept(
                        ...
                        DateTimeFormatHelper.secondsToDHMS(Math.ceil(battleCooldownDifference / 1000))
                    )
                });
            } else {
                interaction.editReply({
                    content: MessageCreator.createAccept(
                        ...
                        interaction.options.getUser("user") ? "The user is" : "You are"
                    )
                });
            }
        ...
        case "join":
            ...
            if (oldJoinCooldownDifference > 0) {
                ...
                responses.push(MessageCreator.createAccept(
                    ...
                    interaction.options.getUser("user") ? "The user is" : "You are",
                    interaction.options.getUser("user") ? "the user's" : "your"
                ));
            }
            ...
            if (joinCooldownDifference > 0) {
                responses.push(MessageCreator.createAccept(
                    ...
                    interaction.options.getUser("user") ? "The user" : "You",
                    DateTimeFormatHelper.secondsToDHMS(Math.ceil(joinCooldownDifference / 1000)).toString()
                ));
            } else {
                responses.push(MessageCreator.createAccept(
