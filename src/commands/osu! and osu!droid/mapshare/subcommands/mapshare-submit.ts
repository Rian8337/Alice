...
export const run: Subcommand["run"] = async (_, interaction) => {
    ...
    if (beatmapInfo.totalDifficulty < 3) {
        return interaction.editReply({
            content: MessageCreator.createReject(mapshareStrings.beatmapIsTooEasy)
        });
    }

    if (beatmapInfo.objects < 50) {
        return interaction.editReply({
            content: MessageCreator.createReject(mapshareStrings.beatmapHasLessThan50Objects)
        });
    }

    if (beatmapInfo.circles + beatmapInfo.sliders === 0) {
        return interaction.editReply({
            content: MessageCreator.createReject(mapshareStrings.beatmapHasNoCirclesOrSliders)
        });
    }

    if (beatmapInfo.hitLength < 30) {
        return interaction.editReply({
            content: MessageCreator.createReject(mapshareStrings.beatmapDurationIsLessThan30Secs)
        });
    }

    if (beatmapInfo.approved === rankedStatus.WIP || beatmapInfo.approved === rankedStatus.QUALIFIED) {
        return interaction.editReply({
            content: MessageCreator.createReject(mapshareStrings.beatmapIsWIPOrQualified)
        });
    }

    if (beatmapInfo.approved !== rankedStatus.RANKED) {
        if (DateTimeFormatHelper.getTimeDifference(beatmapInfo.submitDate) > -86400 * 1000 * 7) {
            return interaction.editReply({
                content: MessageCreator.createReject(mapshareStrings.beatmapWasJustSubmitted)
            });
        }

        if (DateTimeFormatHelper.getTimeDifference(beatmapInfo.lastUpdate) > -86400 * 1000 * 3) {
            return interaction.editReply({
                content: MessageCreator.createReject(mapshareStrings.beatmapWasJustUpdated)
            });
        }
    }
    ...
    if (submission) {
        return interaction.editReply({
            content: MessageCreator.createReject(mapshareStrings.beatmapHasBeenUsed)
        });
    }
    ...
    if (wordCount < 50 || wordCount > 120) {
        return interaction.editReply({
            content: MessageCreator.createReject(
                mapshareStrings.summaryWordCountNotValid, wordCount.toLocaleString()
            )
        });
    }
    ...
    if (summary.length < 100 || summary.length > 900) {
        return interaction.editReply({
            content: MessageCreator.createReject(
                mapshareStrings.summaryCharacterCountNotValid, summary.length.toLocaleString()
            )
        });
    }

    const playerInfo: PlayerInfo | null =
        await DatabaseManager.aliceDb.collections.playerInfo.getFromUser(interaction.user);
    ...
    if (!result.success) {
        return interaction.editReply({
            content: MessageCreator.createReject(
                mapshareStrings.submitFailed, result.reason!
            )
        });
    }
