...
export const run: Command["run"] = async (_, interaction) => {
    ...
    if (result.statusCode !== 200) {
        return interaction.editReply({
            content: MessageCreator.createReject(gamestatsStrings.cannotRetrieveGameStatistics)
        });
    }

    const data: number[] = result.data.toString("utf-8").split("<br>").map(v => parseInt(v));
    ...
    const embed: MessageEmbed = EmbedCreator.createNormalEmbed(
        { author: interaction.user, color: (<GuildMember | null> interaction.member)?.displayHexColor }
    );

    embed.setTitle("Overall Game Statistics")
        .addField(
            "Registered Accounts",
