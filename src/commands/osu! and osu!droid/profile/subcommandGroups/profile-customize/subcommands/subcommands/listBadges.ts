...
export const run: Subcommand["run"] = async (_, interaction) => {
    const badgeList: Collection<string, ProfileBadge> = await DatabaseManager.aliceDb.collections.profileBadges.getAllBadgesSorted();

    const playerInfo: PlayerInfo | null = await DatabaseManager.aliceDb.collections.playerInfo.getFromUser(interaction.user);

    const userBadges: PartialProfileBackground[] = playerInfo?.picture_config.badges ?? [];
    ...
    for (const badge of badgeList.values()) {
        finalBadgeList.push(
            Object.defineProperty(
                ...
                "isOwned",
                { value: !!userBadges.find(v => v.id === badge.id), configurable: true, enumerable: true, writable: true }
            )
        );
    }
    ...
    const onPageChange: OnButtonPageChange = async (options, page, contents: ProfileBadgeOwnerInfo[]) => {
        ...
        for (let i = 5 * (page - 1); i < Math.min(contents.length, 5 + 5 * (page - 1)); ++i) {
            ...
            embed.addField(
                `${i + 1}. ${c.name} (\`${c.id}\`${c.isOwned ? ", owned" : ""})`,
