...
export const run: Subcommand["run"] = async (_, interaction) => {
    ...
    const color: string | undefined = await MessageInputCreator.createInputDetector(
        interaction,
    ...
    if (color.includes(",")) {
        ...
        if (RGBA.length !== 4) {
            return interaction.editReply({
                content: MessageCreator.createReject(profileStrings.invalidRGBAformat)
            });
        }

        if (RGBA.slice(0, 3).some(v => !NumberHelper.isNumberInRange(v, 0, 255, true))) {
            return interaction.editReply({
                content: MessageCreator.createReject(profileStrings.invalidRGBAformat)
            });
        }
