...
async function getBeatmaps(fetchAttempt: number = 0): Promise<MapInfo[]> {
    ...
    const finalDate: Date = new Date(dateBaseLimit + Math.floor(Math.random() * (Date.now() - dateBaseLimit)));
    ...
    data.forEach(v => {
        ...
        if (!beatmapList.find(map => map.beatmapsetID === beatmapInfo.beatmapsetID)) {
...
function createEmbed(level: number, beatmapInfo: MapInfo, artist: string, title: string): MessageEmbed {
    ...
    embed.setAuthor("Beatmap Hint")
        ...
        .setImage(`https://assets.ppy.sh/beatmaps/${beatmapInfo.beatmapsetID}/covers/cover.jpg`);
...
export const run: Subcommand["run"] = async (_, interaction) => {
    if (CacheManager.stillHasMapTriviaActive.has(interaction.channelId)) {
        return interaction.editReply({
            content: MessageCreator.createReject(triviaStrings.channelHasMapTriviaActive)
        });
    }
    ...
    while (!hasEnded) {
        let beatmapInfoIndex: number = beatmapCache.findIndex(v => v.plays <= 10000000 - 200 * level || v.favorites <= 1000000 - 20 * level);
        ...
        while (beatmapInfoIndex === -1) {
            ...
            beatmapInfoIndex = beatmapCache.findIndex(v => v.plays <= 10000000 - 200 * level || v.favorites <= 1000000 - 20 * level);
        }

        if (beatmapInfoIndex === -1) {
            await interaction.channel!.send({
                content: MessageCreator.createReject(triviaStrings.couldNotRetrieveBeatmaps)
            });

            break;
        }

        const beatmapInfo: MapInfo = beatmapCache.splice(beatmapInfoIndex, 1)[0];
        ...
        while (buttons.length < 25) {
            const char: string = String.fromCharCode(65 + Math.floor(Math.random() * 26));
        ...
        buttons.splice(Math.floor(Math.random() * buttons.length), 0, firstButton);
        ...
        for (let i = 0; i < Math.floor(buttons.length / 5); ++i) {
            components.push(
                new MessageActionRow()
                    .addComponents(buttons.slice(5 * i, 5 + 5 * i))
            );
        }
        ...
        await new Promise<void>(resolve => {
            collector.on("collect", async i => {
                ...
                // Disable button
                const button: MessageButton = buttons.find(b => b.customId === char)!;
                ...
                const artistStringDataIndex: number = artistGuessData.replacedStrings.findIndex(t => t.char === char);
                const titleStringDataIndex: number = titleGuessData.replacedStrings.findIndex(t => t.char === char);

                if (artistStringDataIndex === -1 && titleStringDataIndex === -1) {
                    --playerStats.lives;
                ...
                if (artistGuessData.replacedStrings.length > 0) {
                    const artistStringData = artistGuessData.replacedStrings.splice(artistStringDataIndex, 1)[0];

                    for (const index of artistStringData.indexes) {
                        artistGuessData.splittedString[index] = beatmapInfo.artist.charAt(index);
                    }
                }

                if (titleGuessData.replacedStrings.length > 0) {
                    const titleStringData = titleGuessData.replacedStrings.splice(titleStringDataIndex, 1)[0];

                    for (const index of titleStringData.indexes) {
                        titleGuessData.splittedString[index] = beatmapInfo.title.charAt(index);
                    }
                }
            ...
            collector.on("end", async () => {
                ...
                embed.setAuthor("Beatmap Information")
                    .setTitle(`${beatmapInfo.artist} - ${beatmapInfo.title} by ${beatmapInfo.creator}`)
                    .setURL(`https://osu.ppy.sh/s/${beatmapInfo.beatmapsetID}`)
                    .setDescription(`${beatmapInfo.showStatistics(1)}\n${beatmapInfo.showStatistics(4)}\n${beatmapInfo.showStatistics(5)}`)
    ...
    embed.setTitle("Game Information")
        .setDescription(
        ...
        .addField(
            "Leaderboard",
            [...statistics.values()].slice(0, 10).map((v, i) => `**#${i + 1}**: <@${v.id}>: ${v.score.toFixed(2)}`).join("\n") || "None"
        );
