import { Collection, CommandInteraction, GuildMember, MessageCollector, MessageEmbed, Snowflake } from "discord.js";
 ...
 */
function endGame(interaction: CommandInteraction, mode: MathGameType, gameStats: Collection<Snowflake, number>, level: number, operatorCount: number, endMessage: string): void {
    gameStats.sort((a, b) => {
        return b - a;
    });
...
export const run: Command["run"] = async (_, interaction) => {
    ...
    generateEquation(level, operatorAmount, async function createCollector(mathEquation) {
        ...
        if (isNaN(answer) || prevEquations.includes(realEquation)) {
            if (fetchAttempt < 5) {
                ++fetchAttempt;
                return generateEquation(level, operatorAmount, createCollector);
            }

            const endString: string = StringHelper.formatString(mathgameStrings.couldNotFetchEquationGameEnd, (fetchAttempt * 500).toString());

            return endGame(interaction, mode, gameStats, level, operatorAmount, endString);
        }
        ...
        if (!interaction.replied) {
            await interaction.editReply({
                content: MessageCreator.createAccept(mathgameStrings.gameStartedNotification)
            });
        }
        ...
        interaction.channel!.send(questionString).then(msg => {
            ...
            collector.on("collect", m => {
                ...
                msg.channel.send(MessageCreator.createAccept(
            ...
            collector.on("end", async () => {
                if (!correct) {
                    ...
                    const endString: string = StringHelper.formatString(
                        ...
                        mode === "multi" ? "Game ended" : `${interaction.user}, game ended`,
                        realEquation,
                        answer.toString()
                    );
    ...
    options: [
        {
            ...
            description: "The game mode, whether to play by yourself or with others. Defaults to singleplayer."
        }
    ],
    example: [
        {
