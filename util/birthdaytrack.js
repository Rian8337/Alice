    ...
    birthday.find({}).toArray((err, res) => {
        ...
        for (timezone; timezone < 13; ++timezone) {
            ...
            if (isLeapYear) {
                ...
                entries = res.filter((entry) => entry.date === date && entry.month === month && entry.timezone === timezone && entry.isLeapYear === isLeapYear)
            } else entries = res.filter((entry) => entry.date === date && entry.month === month && entry.timezone === timezone);
        }
