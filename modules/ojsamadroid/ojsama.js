...
(function () {
    ...
    std_diff.prototype._length_bonus = function (stars, difficulty) {
        return (
            0.32 + 0.5 * (Math.log10(difficulty + stars) - Math.log10(stars))
        );
    };
    ...
    std_diff.prototype.calc = function (params) {
        ...
        this._init_objects(this.objects, map, map.cs);
        ...
        for (var i = 1; i < this.objects.length; ++i) {
            ...
            var interval = (obj.time - prev.time) / params.speed_mul;
        ...
        switch (type) {
                ...
                if (angle !== null && angle > AIM_ANGLE_BONUS_BEGIN) {
                    angle_bonus = Math.sqrt(
                        Math.max(prev_distance - ANGLE_BONUS_SCALE, 0.0) *
                            Math.pow(
                                Math.sin(angle - AIM_ANGLE_BONUS_BEGIN),
                                2.0
                            ) *
            ...
            case DIFF_SPEED: {
                ...
                if (delta_time < MIN_SPEED_BONUS) {
                    speed_bonus += Math.pow(
                        (MIN_SPEED_BONUS - delta_time) / 50.0,
                        2
                    );
                }
                angle_bonus = 1.0;
                if (angle !== null && angle < SPEED_ANGLE_BONUS_BEGIN) {
                    ...
                    if (angle < Math.PI / 2.0) {
                        angle_bonus = 1.28;
                        if (
                            distance < ANGLE_BONUS_SCALE &&
                            angle < Math.PI / 4.0
                        ) {
                            angle_bonus +=
                                (1.0 - angle_bonus) *
                                Math.min(
                                    (ANGLE_BONUS_SCALE - distance) / 10.0,
                                    1.0
                                );
                ...
                return (
                    ((1 + (speed_bonus - 1) * 0.75) *
                        ...
                        (0.95 +
                            speed_bonus *
                                Math.pow(distance / SINGLE_SPACING, 3.5))) /
                    strain_time
                );
            }
        }
    ...
    std_ppv2.prototype.calc = function (params) {
        ...
        if ([aim_stars, speed_stars].some(isNaN)) {
            throw new TypeError(
                "aim and speed stars required (must be numbers)"
            );
        }
        ...
        var extreme_penalty = Math.pow(
            1 -
                Math.abs(speed - Math.pow(aim, 1.1)) /
